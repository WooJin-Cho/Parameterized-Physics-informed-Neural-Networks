...
parser.add_argument('--system', type=str, default='cdr', help='System to study.')
...
parser.add_argument('--u0_str', default='gauss_pi_2')
...
os.makedirs('./dataset/cdr/train', exist_ok=True)
os.makedirs('./dataset/cdr/val', exist_ok=True)
os.makedirs('./dataset/cdr/test', exist_ok=True)
    ...
    for i in range(41):
        ...
        for j in range(41):
            ...
            if 'convection' in args.system or 'diffusion' in args.system:
                u_vals, u_v = convection_diffusion(args.u0_str, nu, beta, args.source, args.xgrid, args.nt)
                G = np.full(X_f_train.shape[0], float(args.source))         
            elif 'cdr' in args.system:
                u_vals, u_v = convection_diffusion_reaction_discrete_solution(args.u0_str, beta, nu, rho, args.xgrid, args.nt)
                G = np.full(X_f_train.shape[0], float(args.source))
            ...
            X_test_arr  = np.concatenate((X_test, u_test, X_test_beta, X_test_nu, X_test_rho), axis=1)
            ...
            train_boundary_file_name = './dataset/cdr/train/train_boundary_' + str(int(beta)) + '_' + str(int(nu)) + '_' + str(int(rho)) + '_cdr.csv'
            train_u_file_name        = './dataset/cdr/train/train_u_' + str(int(beta)) + '_' + str(int(nu)) + '_' + str(int(rho)) + '_cdr.csv'
            train_f_file_name        = './dataset/cdr/train/train_f_' + str(int(beta)) + '_' + str(int(nu)) + '_' + str(int(rho)) + '_cdr.csv'
            test_file_name           = './dataset/cdr/test/test_' + str(int(beta)) + '_' + str(int(nu)) + '_' + str(int(rho)) + '_cdr.csv'
            val_file_name            = './dataset/cdr/val/val_' + str(int(beta)) + '_' + str(int(nu)) + '_' + str(int(rho)) + '_cdr.csv'
