...
for i in range(41):
    ...
    if 'convection' in args.system or 'diffusion' in args.system:
        ...
        u_vals, u_v = reaction_solution(args.u0_str, rho, args.xgrid, args.nt)
        G = np.full(X_f_train.shape[0], float(args.source))
    else:
    ...
    train_boundary_file_name = './dataset/reaction/train/train_boundary_' + str(int(rho)) + '_reaction.csv'
    train_u_file_name        = './dataset/reaction/train/train_u_' + str(int(rho)) + '_reaction.csv'
    train_f_file_name        = './dataset/reaction/train/train_f_' + str(int(rho)) + '_reaction.csv'
    test_file_name           = './dataset/reaction/test/test_' + str(int(rho)) + '_reaction.csv'
    val_file_name            = './dataset/reaction/val/val_' + str(int(rho)) + '_reaction.csv'
