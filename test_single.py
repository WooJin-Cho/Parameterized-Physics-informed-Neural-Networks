...
def main():
    ...
    print(f"- {pde_type: <10} w/ coeff {coefficient_range} : {np.mean(np.array(l2_abs_errors)):.4f}, {np.mean(np.array(l2_rel_errors)):.4f}, {np.mean(np.array(max_errs)):.4f}, {np.mean(np.array(ex_vars)):.4f}")
